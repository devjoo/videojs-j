...
module.exports = {
		  ...
		  console.log(preroll);
			if (preroll) {
				...
				if (preroll.video) {
					...
					console.log(player);
          player.preroll({
            src:{src:"//cdn.nuevolab.com/media/preroll.mp4",type:"video/mp4"},
            href:"https://www.nuevolab.com",
            skipTime:5
          });
    ...
    vastClient.get(url).then(response => {
        if (timedOut) {
          return;
        }
        ...
        if (response) {
          response.ads.forEach(function (ad) {
            ad.creatives.some(function (creative) {
              ...
              var tracker = new vast.VASTTracker(vastClient, ad, creative);
